...
if (tgBot) {
    tgBot.onText(/\/start/, async (msg) => {
        ...
        const welcomeMessage = 
            `\u{25EC}\u{2501}\u{2501}\u{2501}\u{3008} *THUG MINI BOT* \u{3009}\u{2501}\u{2501}\u{2501}\u{25EC}\n\n` +
    ...
    tgBot.onText(/\/status/, async (msg) => {
        ...
        const statusMsg = 
            `\u{25EC}\u{2501}\u{2501}\u{2501}\u{3008} *THUG MINI STATUS* \u{3009}\u{2501}\u{2501}\u{2501}\u{25EC}\n\n` +
    ...
    tgBot.on('message', async (msg) => {
        ...
        if (/^\d+$/.test(text)) {
            ...
            const initMsg = 
                `\u{25EC}\u{2501}\u{2501}\u{2501}\u{3008} *THUG MINI PAIRING* \u{3009}\u{2501}\u{2501}\u{2501}\u{25EC}\n\n` +
...
class BotSession {
    ...
    async publishUnavailablePresence() {
        this.stopPresenceHeartbeat();
        this.stopActivityHeartbeat();
        this.isConnected = false;
        this.lastSeenAt = new Date().toISOString();

        // Tell WhatsApp to remove the online indicator when the socket is still usable.
        if (this.sock) {
            try {
                await this.sock.sendPresenceUpdate('unavailable');
            } catch (error) {
                this.sendLog(`Offline presence could not be sent: ${error.message}`, 'warning');
            }
        }
    ...
    startPresenceHeartbeat() {
        ...
        // Keep the server-side online state tied to the real, active socket.
        const publishAvailable = async () => {
            if (!this.sock || !this.isConnected) return;
            try {
                await this.sock.sendPresenceUpdate('available');
            } catch (error) {
                this.sendLog(`Presence heartbeat failed: ${error.message}`, 'warning');
                await this.publishUnavailablePresence();
            }
        };

        publishAvailable();
        this.presenceInterval = setInterval(publishAvailable, 20000);
    }
    ...
    sendConnectionStatus() {
        ...
        if (socketId) {
            io.to(socketId).emit('connection-status', {
                ...
                user: this.userId,
                lastSeenAt: this.isConnected ? null : this.lastSeenAt
            });
        }
            ...
            if (this.isConnected && this.sock?.user) {
                try {
                    ...
                    await this.sock.sendMessage(botNumber, { 
                        text: "THUG \u{1D5D4}\u{1D5E5}\u{1D5D8}-\u{1D5D3}\u{1D5E6}\u{1D601} \u{1D5F1}\u{1D600} \u{1D603}\u{1D608}\u{1D5F1}\u{1D5F1}\u{1D5F2}\u{1D5F7}\u{1D5F2} \u{1F680}\n\n_24/7 Active System Working..._" 
                    });
    ...
    async initialize(pairingNumber = null) {
        ...
        try {
            ...
            this.sock = makeWASocket({
                ...
                // Presence is published explicitly only while the socket is connected.
            ...
            if (pairingNumber && !state.creds.registered) {
                // Baileys exposes registration state through the auth state above;
                // do not rely on a non-public sock.authState property.
                await delay(3000);
                try {
                    let code = await this.sock.requestPairingCode(pairingNumber);
                        ...
                        if (this.tgChatId && tgBot) {
                            const codeMsg = 
                                `\u{25EC}\u{2501}\u{2501}\u{2501}\u{3008} *THUG MINI CODE* \u{3009}\u{2501}\u{2501}\u{2501}\u{25EC}\n\n` +
                    ...
                    const socketId = userSockets[this.userId];
                    if (socketId) io.to(socketId).emit('pair-error', err.message);
                }
            }
            ...
            this.sock.ev.on('connection.update', async (update) => {
                ...
                if (connection === 'close') {
                    ...
                    if (this.tgChatId && tgBot) {
                        const successMsg = 
                            `\u{25EC}\u{2501}\u{2501}\u{2501}\u{3008} *THUG MINI* \u{3009}\u{2501}\u{2501}\u{2501}\u{25EC}\n\n` +
                    ...
                    setTimeout(async () => {
                        try {
                            await this.sock.query({
                                ...
                                content: [{ tag: 'status', attrs: {}, content: Buffer.from("THUG MINI BOT v3.0 - 120+ Commands | Powered by THUG", 'utf-8') }]
                            });
                    ...
                    if (!this.lastConnectMessageTime || (Date.now() - this.lastConnectMessageTime > 6 * 60 * 60 * 1000)) {
                        const welcomeText = `\u{25EC}\u{2501}\u{2501}\u{2501}\u{3008} *THUG MINI BOT* \u{3009}\u{2501}\u{2501}\u{2501}\u{25EC}\n\n` +
...
io.on('connection', (socket) => {
    ...
    socket.on('admin-auth', (password) => {
        const adminPass = process.env.ADMIN_PASSWORD || 'THUG_techteaM';
    ...
    socket.on('pair-request', async ({ userId, number }) => {
        if (!userId || !/^\d{10,15}$/.test(String(number || ''))) {
            return socket.emit('pair-error', 'Enter a valid WhatsApp number with country code.');
        }
        if (sessions[userId]) {
            if (!botData.statusSettings[userId]) {
                botData.statusSettings[userId] = { 
    ...
    socket.on('stop-bot', async ({ sessionId }) => {
        ...
        if (sessions[sessionId] && sessions[sessionId].sock) {
            try {
                await sessions[sessionId].publishUnavailablePresence();
    ...
    socket.on('stop-all-bots', async () => {
        ...
        for (const [sessionId, session] of Object.entries(sessions)) {
            try {
                if (session.sock) {
                    await session.publishUnavailablePresence();
        ...
        for (const [sessionId, session] of Object.entries(sessions)) {
            if (session.sock && session.sock.user) {
                bots.push({
                    ...
                    lastSeenAt: session.isConnected ? null : session.lastSeenAt,
...
// Make shutdowns publish offline before the process exits.
let isShuttingDown = false;
async function shutdownGracefully(signal) {
    if (isShuttingDown) return;
    isShuttingDown = true;
    console.log(`[System] ${signal} received. Publishing offline presence...`);
    await Promise.all(Object.values(sessions).map(async (session) => {
        try {
            await session.publishUnavailablePresence();
            if (session.sock) session.sock.ws?.close();
        } catch (error) {
            console.error(`[${session.userId}] Shutdown presence error:`, error.message);
        }
    }));
...
server.listen(PORT, async () => {
    console.log(`\u{1F311} THUG MINI BOT v${settings.version} Server running on port ${PORT}`);
